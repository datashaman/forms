...
import { useFetch } from "#app"; // Nuxt 3 composable for server-side API calls
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      const params = {
        ...
        responseType: "stream",
        tools: tools.defineTools(["getCurrentTime"]),
      };

      const response = await $fetch("/api/openai", params);
